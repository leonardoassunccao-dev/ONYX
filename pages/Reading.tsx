  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <header className="flex justify-between items-center">
        ...
        <button onClick={() => setShowForm(!showForm)} className="bg-[#C0C0C0] hover:bg-white text-[#0b0b0d] p-3 rounded-xl transition-all">
          <Plus size={20} />
        </button>
      </header>
        ...
        <Card accentBorder title="Nova Operação de Leitura">
          <form onSubmit={handleAdd} className="space-y-4">
            <input 
              ...
              className="w-full bg-[#121212] border border-zinc-800 rounded-xl p-3 text-white focus:border-[#C0C0C0] outline-none text-sm font-bold uppercase tracking-widest"
              value={newTitle}
            ...
            />
            <button type="submit" className="w-full bg-[#C0C0C0] text-[#0b0b0d] py-3 font-black tracking-widest rounded-xl text-xs uppercase hover:bg-white transition-all">Ativar Protocolo</button>
          </form>
        </Card>
      )}
                ...
                <div className="flex flex-col md:flex-row gap-6">
                  <div className="flex-1 space-y-4">
                    <div className="flex items-start gap-4">
                      <div className="p-4 rounded-xl bg-[#121212] text-[#C0C0C0] border border-zinc-800">
                        <BookOpen size={24} />
                      </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                       <div className="bg-[#0B0B0B] p-3 border border-zinc-800/50 rounded-xl">
                  ...
                  <div className="md:w-64 bg-[#121212] border border-zinc-800 rounded-2xl p-4 flex flex-col justify-between">
                    {projection ? (
                    ...
                    <button 
                      ...
                      className="mt-4 w-full bg-zinc-900 border border-zinc-800 py-2 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] text-[#C0C0C0] hover:bg-zinc-800"
                    >
                      Logar Leitura
                    </button>
                  </div>
                ...
                {loggingBookId === book.id && (
                  <form onSubmit={(e) => handleLog(e, book.id!)} className="mt-4 pt-4 border-t border-zinc-800 flex gap-2 animate-in slide-in-from-top-2">
                    <input 
                      ...
                      className="flex-1 bg-[#0B0B0B] border border-zinc-800 rounded-xl p-2 text-xs text-white focus:border-[#C0C0C0] outline-none"
                      value={logPages}
                      onChange={e => setLogPages(e.target.value)}
                    />
                    <button type="submit" className="bg-[#C0C0C0] text-[#0b0b0d] px-6 rounded-xl text-[10px] font-black uppercase tracking-widest">Consolidar</button>
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {archiveBooks.map(book => (
              <div key={book.id} className="bg-[#0B0B0B] border border-[#1a1a1a] p-4 rounded-2xl flex items-center gap-4 group">
                <div className="p-3 bg-zinc-900 text-zinc-700 rounded-xl">
                  <BookOpen size={18} />
                </div>
                <div className="flex-1 min-w-0">
      ...
      {books.length === 0 && !showForm && (
        <div className="text-center py-24 bg-[#0B0B0B]/20 border border-dashed border-zinc-800 rounded-2xl flex flex-col items-center">
